...
# ════════════════════════════════════════════════════════════════
#  CODE_GENERATE — multi-file agentic build/edit, ported from the
#  CodeAgent project's structured-JSON contract + diff-apply engine.
#  Difference from CodeAgent: there files live in the browser's
#  localStorage and get applied client-side; here they live in a
#  GitHub repo, so "apply" means fetch current content -> apply
#  edits/creates in memory -> push everything as ONE commit via the
#  Git Data API (same blobs->tree->commit->ref pattern /upload-zip
#  already uses, factored out below so both share it).
# ════════════════════════════════════════════════════════════════
